...
if __name__ == '__main__':
    ...
    # pcd_arr = os.path.join(path, r'../../data/pcd_data.npy')
    path_m_images = os.path.join(path, r'../best_model_image.pth')
    # path_m_pcd = 'model_pcd.pth'
    ...
    model_images = torch.load(path_m_images)
    model_images.to(device)
    feature_images = []
    for image in loader_images:
        image = image.to(device)
        mu_encode, log_var = model_images.encode(image)
        feature = model_images.reparameterize(mu = mu_encode, logvar = log_var)
        feature = feature.cpu().detach().numpy()
        feature_images.append(feature)
    feature_images = np.vstack(feature_images)


    print ('Shape of image features: ')
    ...
    # print ('Shape of image features after PCA: ')
    # print (fit.shape)
    
    np.save('features.npy', feature_images) # save
